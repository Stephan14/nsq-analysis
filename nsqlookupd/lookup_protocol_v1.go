...
type LookupProtocolV1 struct { //实现protocal接口
	ctx *Context
}
...
func (p *LookupProtocolV1) IOLoop(conn net.Conn) error {
	...
	reader := bufio.NewReader(client) //TODO bufio 使用
	for {                             //长连接
		...
		if response != nil { // response 类型为slice
...
func getTopicChan(command string, params []string) (string, string, error) { //返回多个变量不定义变量名
	if len(params) == 0 {
	...
	if !protocol.IsValidTopicName(topicName) { //利用正则表达式判断字符串
		return "", "", protocol.NewFatalClientErr(nil, "E_BAD_TOPIC", fmt.Sprintf("%s topic name '%s' is not valid", command, topicName))
	}

	if channelName != "" && !protocol.IsValidChannelName(channelName) { //利用正则表达式判断字符
...
func (p *LookupProtocolV1) IDENTIFY(client *ClientV1, reader *bufio.Reader, params []string) ([]byte, error) {
	...
	var bodyLen int32
	err = binary.Read(reader, binary.BigEndian, &bodyLen) //获取消息的长度
	if err != nil {
		return nil, protocol.NewFatalClientErr(err, "E_BAD_BODY", "IDENTIFY failed to read body size")
	}

	body := make([]byte, bodyLen)
	_, err = io.ReadFull(reader, body) //读取整个消息体
	if err != nil {
